...
if "Consumidor" in visao:
    ...
    st.markdown("<div class='price-table-wrapper'>", unsafe_allow_html=True)
    for i, row in sub.iterrows():
        is_best = row["Preço (R$)"] == menor_preco
        badge = "<span class='badge-best'>Melhor Preço</span>" if is_best else ""
        cls = "price-row best" if is_best else "price-row"
        st.markdown(f"""
        <div class='{cls}'>
          <div>
            <div class='market-name'>{row['Mercado']}</div>
            <div class='market-dist'>📍 {row['Distância']}
              <span class='badge-update' style='margin-left:10px'>⏱ {row['Timestamp']}</span>
            </div>
          </div>
          <div style='display:flex;align-items:center;gap:12px'>
            {badge}
            <div class='price-val'>R$ {row['Preço (R$)']:.2f}</div>
          </div>
        </div>""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    fig.add_trace(go.Scatter(
        x=hist_df["Data"],
        y=hist_df["Mercado Boa Vista"],
        mode="lines+markers",
        name="Seu Mercado",
        line=dict(color="#00c882", width=3),
        marker=dict(size=7, color="#00c882"),
        fill="tozeroy",
        fillcolor="rgba(0,200,130,0.06)",
    ))
    fig.add_trace(go.Scatter(
        x=hist_df["Data"],
        y=hist_df["Média do Bairro"],
        mode="lines+markers",
        name="Média do Bairro",
        line=dict(color="#7fb8d4", width=2, dash="dot"),
        marker=dict(size=5, color="#7fb8d4"),
    ))
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(13,58,92,0.2)",
        font=dict(family="Sora", color="#c8e6f5", size=12),
        legend=dict(
            bgcolor="rgba(13,58,92,0.6)",
            bordercolor="rgba(0,200,130,0.3)",
            borderwidth=1,
            font=dict(size=11),
        ),
        xaxis=dict(
            gridcolor="rgba(13,58,92,0.8)",
            color="#7fb8d4",
            showgrid=True,
        ),
        yaxis=dict(
    ...
    sub_b2b = DF[DF["Produto"] == produto_b2b].sort_values("Preço (R$)")
    colors = ["#00c882" if m == "Mercado Boa Vista" else "#1a5a82" for m in sub_b2b["Mercado"]]

    fig2 = go.Figure(go.Bar(
        x=sub_b2b["Mercado"],
        y=sub_b2b["Preço (R$)"],
        marker_color=colors,
        text=[f"R$ {v:.2f}" for v in sub_b2b["Preço (R$)"]],
        textposition="outside",
        textfont=dict(color="#c8e6f5", size=12, family="JetBrains Mono"),
    ))
    fig2.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(13,58,92,0.2)",
        font=dict(family="Sora", color="#c8e6f5", size=11),
        xaxis=dict(gridcolor="rgba(13,58,92,0.5)", color="#7fb8d4", tickangle=-20),
        yaxis=dict(gridcolor="rgba(13,58,92,0.5)", color="#7fb8d4", tickprefix="R$ "),
